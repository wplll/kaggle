...
class LayerNormChannel(nn.Module):
    """
    LayerNorm only for Channel channel_mg Dimension.
...
class BaseConv(nn.Module):
    """A Conv2d -> Batchnorm -> silu/leaky relu block -> cbl""" # CBL
...
class ResLayer(nn.Module):
    "Residual layer with `in_channels_mg` inputs."
...
class CSPLayer(nn.Module):
    """C3 in yolov5, CSP Bottleneck with 3_1 convolutions"""
...
class EVCBlock(nn.Module):
    def __init__(self, in_channels, out_channels, channel_ratio=4, base_channel=16):
        ...
        # LVC
        self.lvc = LVCBlock(in_channels=in_channels, out_channels=out_channels, num_codes=64)  # c1值暂时未定时
        # LightMLPBlock
